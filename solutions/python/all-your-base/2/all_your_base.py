...
def _to_base_ten(digits: list[int], base: int) -> int:
    ...
    for num in digits:
        ...
        output = (output * base) + num  # use Horner's Method
    return output
...
def _base_ten_to_other(num: int, base: int) -> list[int]:
    ...
    output: list[int] = []
    while num > 0:
        num, rem = divmod(num, base)  # use divmod to get both reminder and questiont.
        output.append(rem)  # append and inverse, rather than insert.
    output.reverse()
    return output or [0]
